...
s1_list = ['E12_5-S1', 'E13_5-S1', 'E15_5-S1']
s2_list = ['E12_5-S2', 'E13_5-S2', 'E15_5-S2']
slice_name_list = [s1_list, s2_list]
...
for j, slice_list in enumerate(slice_name_list):

    # load dataset
    cas_list = [ad.read_h5ad(data_dir + sample + '.h5ad') for sample in slice_list]
    for i in range(len(cas_list)):
        cas_list[i].obs_names = [x + '_' + slice_list[i] for x in cas_list[i].obs_names]

    # concatenation
    adata_concat = ad.concat(cas_list, label="slice_name", keys=slice_list)

    # preprocess CAS data
    # peaks are already merged and fragment counts are stored in the data matrices
    print('Start preprocessing')
    preprocess_CAS(cas_list, adata_concat, min_cells_rate=0.003)  # s1:112732 s2:116259
    print('Done!')
    print(adata_concat)

    adata_concat.write_h5ad(save_dir + f"preprocessed_concat_S{j+1}.h5ad")
    for i in range(len(slice_list)):
        cas_list[i].write_h5ad(save_dir + f"filtered_{slice_list[i]}.h5ad")

    cas_list = [ad.read_h5ad(save_dir + f"filtered_{sample}.h5ad") for sample in slice_list]
    adata_concat = ad.read_h5ad(save_dir + f"preprocessed_concat_S{j+1}.h5ad")

    print(f'Applying PCA to reduce the feature dimension to 100 ...')
    pca = PCA(n_components=100, random_state=1234)
    input_matrix = pca.fit_transform(adata_concat.X.toarray())
    np.save(save_dir + f'input_matrix_S{j+1}.npy', input_matrix)
    print('Done !')
    ...
    for i in range(len(cas_list)):
        ...
        if slice_list[i] == 'E12_5-S1' or slice_list[i] == 'E12_5-S2':
            size = 20
        else:
            size = 15
        axs[j, i].scatter(cas_list[i].obsm['spatial'][:, 1], cas_list[i].obsm['spatial'][:, 0],
                          linewidth=0.5, s=size, marker=".", color=real_colors, alpha=0.9)
        axs[j, i].set_title(f'{slice_list[i]}', size=12)
        if slice_list[i] == 'E15_5-S1':
