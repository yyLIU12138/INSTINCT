...
def d_plot_mouseembryo_3(cas_list, adata_concat, clusters_key, method, cluster_to_color_map,
                         slice_name_list, sp_embedding,
                         save_root=None, frame_color=None, save=False, plot=False, file_format='pdf'):

    num_clusters = len(adata_concat.obs[clusters_key].unique())
    print(f'{method} algorithm identifies {num_clusters} unique clusters !')

    fig, axs = plt.subplots(1, 3, figsize=(10, 3))
    fig.suptitle(f'{method} Clustering Results', fontsize=16)
    for i in range(len(cas_list)):
        if slice_name_list[i] == 'E12_5-S1' or slice_name_list[i] == 'E12_5-S2':
            size = 20
        else:
        ...
        if slice_name_list[i] == 'E15_5-S1':
            axs[i].invert_xaxis()
            axs[i].invert_yaxis()
        cluster_colors = list(cas_list[i].obs[clusters_key].map(cluster_to_color_map))
        axs[i].scatter(cas_list[i].obsm['spatial'][:, 1], cas_list[i].obsm['spatial'][:, 0], linewidth=0.5, s=size,
                       marker=".", color=cluster_colors, alpha=0.9)
        axs[i].set_title(f'{slice_name_list[i]}', size=12)
        axs[i].axis('off')
    ...
    legend_handles = [
        Line2D([0], [0], marker='o', color='w', markersize=8, markerfacecolor=cluster_to_color_map[str(i)],
               label=f'{i}') for i in range(num_clusters)
    ]
    axs[2].legend(
        handles=legend_handles,
        fontsize=8, title='Clusters', title_fontsize=10, bbox_to_anchor=(1, 1))
    plt.gcf().subplots_adjust(left=0.05, top=None, bottom=None, right=0.85)
    if save:
        save_path = save_root + f'{method}_clustering_results.{file_format}'
        plt.savefig(save_path)
    ...
    plt.legend(handles=legend_handles, fontsize=8, title='Slices', title_fontsize=10,
               loc='upper left')
    plt.title(f'Slices', fontsize=16)
    if save:
        save_path = save_root + f"slices_umap.{file_format}"
        plt.savefig(save_path)

    colors = list(adata_concat.obs[clusters_key].map(cluster_to_color_map))
    plt.figure(figsize=(5, 5))
    if frame_color:
        plt.rc('axes', edgecolor=frame_color, linewidth=2)
    plt.scatter(sp_embedding[order, 0], sp_embedding[order, 1], s=size, c=colors)
    plt.tick_params(axis='both', bottom=False, top=False, left=False, right=False,
                    labelleft=False, labelbottom=False, grid_alpha=0)
    plt.title(f'Identified Clusters ({method})', fontsize=16)
    if save:
        save_path = save_root + f"{method}_clusters_umap.{file_format}"
        plt.savefig(save_path)
...
def d_plot_mouseembryo_6(cas_list, adata_concat, clusters_key, method, cluster_to_color_map,
                         slice_name_list, sp_embedding,
                         save_root=None, frame_color=None, save=False, plot=False, file_format='pdf'):
