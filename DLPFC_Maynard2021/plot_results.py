...
models = ['INSTINCT', 'SEDR', 'STAligner', 'GraphST']
labels1 = ['INSTINCT', 'SEDR', 'STAligner', 'GraphST']
color_list_1 = ['darkviolet', 'darkslategray', 'c', 'cyan']
...
for idx in range(len(sample_group_list)):
    ...
    for model_name in models:
        ...
        with open(save_dir + f'comparison/{model_name}/{model_name}_group{idx}_results_dict.pkl', 'rb') as file:
    ...
    axs.set_title(f'Clustering Performance (Sample {samples[idx]})', fontsize=14)
    ...
    if save:
        save_path = save_dir + f"comparison/group{idx}_methods_clustering_performance_separate_scores_box_plot.{file_format}"
    ...
    plt.title(f'Representation Quality (Sample {samples[idx]})', fontsize=14)
    ...
    if save:
        save_path = save_dir + f"comparison/group{idx}_methods_representation_quality_separate_scores_box_plot.{file_format}"
        plt.savefig(save_path, dpi=500)
    if show:
        plt.show()
    ...
    plt.title(f'Batch Correction (Sample {samples[idx]})', fontsize=14)
    ...
    if save:
        save_path = save_dir + f"comparison/group{idx}_methods_batch_correction_separate_scores_box_plot.{file_format}"
        plt.savefig(save_path, dpi=500)
    if show:
        plt.show()
    ...
    if save:
        save_path = save_dir + f"comparison/group{idx}_methods_clustering_performance_separate_scores_bar_plot.{file_format}"
        plt.savefig(save_path, dpi=500)
    if show:
        plt.show()
    ...
    if save:
        save_path = save_dir + f"comparison/group{idx}_methods_representation_quality_separate_scores_bar_plot.{file_format}"
        plt.savefig(save_path, dpi=500)
    if show:
        plt.show()
    ...
    if save:
        save_path = save_dir + f"comparison/group{idx}_methods_batch_correction_separate_scores_bar_plot.{file_format}"
        plt.savefig(save_path, dpi=500)
    if show:
        plt.show()
    ...
    for k in range(len(mtx_list)):
        ...
        if save:
            overview_matrix = np.zeros((len(models), len(metric_list)+4), dtype=float)
            if type_list[k] == 'mean':
                ...
                overview_matrix[:, 15] = np.median((np.mean(cluster_stacked, axis=-1) +
                                                    np.mean(bio_conserve_stacked, axis=-1) +
                                                    np.mean(batch_corr_stacked, axis=-1)) / 3, axis=-1)
            np.savetxt(save_dir + f'comparison/group{idx}_methods_{type_list[k]}_separate_scores.txt', overview_matrix)
        ...
        for i in range(len(models)):
            text = plt.text(-1.3, i, labels1[i], ha='center', va='center',
                            fontdict={'size': 12})
        for i in range(len(metric_list)):
            text = plt.text(i, 3.9, metric_list[i], ha='center', va='center', rotation=30,
                            fontdict={'size': 11})
        ...
        if save:
            save_path = save_dir + f"comparison/group{idx}_methods_{type_list[k]}_separate_scores_rank_plot.{file_format}"
            plt.savefig(save_path, dpi=500)
        if show:
            plt.show()
    ...
    for k in range(len(metric_groups)):
        ...
        axs.set_xticklabels(labels1, rotation=20)
        ...
        axs.set_title(f'{metric_groups[k]} (Sample {samples[idx]})', fontsize=14)

        if save:
            save_path = save_dir + f"comparison/group{idx}_methods_{metric_groups[k]}_score_box_plot.{file_format}"
    ...
    for k in range(len(metric_groups)):
        ...
        if save:
            save_path = save_dir + f"comparison/group{idx}_methods_{metric_groups[k]}_score_bar_plot.{file_format}"
            plt.savefig(save_path, dpi=300)
    if show:
        plt.show()
    ...
    axs.set_title(f'Overall Score (Sample {samples[idx]})', fontsize=14)

    if save:
        save_path = save_dir + f"comparison/group{idx}_methods_overall_score_box_plot.{file_format}"
        plt.savefig(save_path, dpi=300)
    if show:
        plt.show()
