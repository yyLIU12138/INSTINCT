...
for model in models:
    ...
    with open(save_dir + f'{model}/{model}_results_dict.pkl', 'rb') as file:
...
for idx, slice_name in enumerate(slice_name_list):
    ...
    for model in models:
        with open(save_dir + f'{model}/{model}_results_dict_{slice_name}.pkl', 'rb') as file:
            results_dict = pickle.load(file)
    ...
    axs.set_title(f'Clustering Performance ({slice_name})', fontsize=16)
    ...
    if save:
        save_path = save_dir + f"{slice_name}_methods_clustering_performance_separate_scores_box_plot.{file_format}"
        plt.savefig(save_path, dpi=500)
    # plt.show()
    ...
    plt.title(f'Representation Quality ({slice_name})', fontsize=16)
    ...
    if save:
        save_path = save_dir + f"{slice_name}_methods_representation_quality_separate_scores_box_plot.{file_format}"
        plt.savefig(save_path, dpi=500)
    # plt.show()
    ...
    if save:
        save_path = save_dir + f"{slice_name}_methods_clustering_performance_separate_scores_bar_plot.{file_format}"
        plt.savefig(save_path, dpi=500)
    # plt.show()
    ...
    if save:
        save_path = save_dir + f"{slice_name}_methods_representation_quality_separate_scores_bar_plot.{file_format}"
        plt.savefig(save_path, dpi=500)
    # plt.show()
    ...
    for k in range(len(metric_groups)):
        ...
        axs.set_title(f'{metric_groups[k]} ({slice_name})', fontsize=16)
        plt.gcf().subplots_adjust(left=0.15, top=None, bottom=0.15, right=None)

        if save:
            save_path = save_dir + f"{slice_name}_methods_{metric_groups[k]}_score_box_plot.{file_format}"
            plt.savefig(save_path, dpi=300)
    plt.show()
    ...
    for k in range(len(metric_groups)):
        ...
        if save:
            save_path = save_dir + f"{slice_name}_methods_{metric_groups[k]}_score_bar_plot.{file_format}"
            plt.savefig(save_path, dpi=300)
    plt.show()
    ...
    axs.set_title(f'Overall Score ({slice_name})', fontsize=16)
    plt.gcf().subplots_adjust(left=0.15, top=None, bottom=0.15, right=None)

    if save:
        save_path = save_dir + f"{slice_name}_methods_overall_score_box_plot.{file_format}"
        plt.savefig(save_path, dpi=300)
    plt.show()
