...
scIB_knit_table <- function(
  ...
  column_pos[ind_text, "xmin"] <- column_pos[ind_text, "xmin"] - 1
  column_pos[ind_text, "xmax"] <- column_pos[ind_text, "xmax"] - 2
  ...
  if (nrow(df) > 0) {
    segment_data <- segment_data %>% bind_rows(
      df %>% transmute(x = x, xend = x, y = -10.5, yend = -10.7, size = .5)
    )
    text_data <-
      bind_rows(
        text_data,
        df %>% transmute(
          xmin = x, xmax = x, ymin = -10.5, ymax = -11,
...
#   # }
  ...
  for(rg in rank_groups){
    rank_palette <- colorRampPalette(rev(brewer.pal(9, palettes[[rg]])))(7)
    
    
    rank_data <- data.frame(xmin = rank_minimum_x[[rg]],
                            xmax = rank_minimum_x[[rg]] + .8,
                            ymin = seq(leg_max_y-6.5, leg_max_y - 3.5, by = .5),
                            ymax = seq(leg_max_y-6, leg_max_y -3, by = .5),
                            border = TRUE,
                            colors = rank_palette
    )
  ...
  arrow_text <- data.frame(xmin = leg_max_x + 1,
                           xmax = leg_max_x + 1.5,
                           ymin = c(leg_max_y - 3.5, leg_max_y - 6.5),
                           ymax = c(leg_max_y - 3, leg_max_y - 6),
...
save_dir <- 'C:/University/FINAL/results/MouseBrain_Jiang2023/'


## column_info
column_id <- c('Name', 'ARI', 'AMI', 'NMI', 'FMI', 'Comp', 'Homo', 'Overall score 1',
               'mAP', 'Spot-type ASW', 'Isolated label ASW', 'Isolated label F1', 'Overall score 2',
               'Batch ASW', 'Batch PCR', 'kBET', 'Graph connectivity', 'Overall score 3', 'Overall score')

column_color_group <- c('Methods', rep('Clustering Performance', 7), 
                        rep('Representation Quality', 5), rep('Batch Correction', 5), 'Overall')

column_geom <- c('text', rep('circle', 6), 'bar', rep('circle', 4), 'bar', rep('circle', 4), 'bar', 'bar')
...
## row_info
row_id <- c('INSTINCT', 'Scanorama', 'SCALEX', 'PeakVI', 'SEDR', 'STAligner', 'GraphST')
...
# mean_11_to_14 <- rowMeans(data_mtx[, 11:14])
# mean_7_to_10 <- rowMeans(data_mtx[, 7:10])
# mean_1_to_6 <- rowMeans(data_mtx[, 1:6])
# mean_1_to_14 <- rowMeans(cbind(mean_11_to_14, mean_7_to_10, mean_1_to_6))

row_names <- c('INSTINCT', 'Scanorama', 'SCALEX', 'PeakVI', 'SEDR', 'STAligner', 'GraphST')
# data_mtx <- cbind(data_mtx[, 1:6], mean_1_to_6, data_mtx[, 7:10], mean_7_to_10, data_mtx[, 11:14], mean_11_to_14, mean_1_to_14)
...
ggsave(paste0(save_dir, 'comparison/methods_mean_overview_table.pdf'), g1, width = 11, height = 7)
ggsave(paste0(save_dir, 'comparison/methods_median_overview_table.pdf'), g2, width = 11, height = 7)
