...
class INSTINCT_Model():
    ...
    def train(self, report_loss=True, report_interval=100, save_loss=False, save_model_params=False,
              save_dir_loss='../../results/', fname1='Loss_values_ae', fname2='Loss_values_whole',
              save_loss_interval=10, save_dir_model_params='../../results/', params_name='INSTINCT_params',
              save_params_interval=100):
        ...
        for step in range(self.training_steps[0]):
            ...
            loss_list.append([loss_g.item(), loss_d.item(), loss_adv_g.item(), loss_adv_d.item(),
            ...
            if save_loss:
                ...
                if (step + 1) % save_loss_interval == 0 or step == 0:
                    with open(save_dir_loss + fname1 + f'_{self.training_steps}.csv', 'w', newline='') as csvfile:
                        csvwriter = csv.writer(csvfile)

                        csvwriter.writerow(['Loss_G', 'Loss_D', 'Loss_Adv_G', 'Loss_Adv_D',
                                            'Loss_Rec', 'Loss_Cls_Real', 'Loss_Cls_Fake'])

                        csvwriter.writerows(loss_list)

            if save_model_params:
                if not os.path.exists(save_dir_model_params):
                    os.makedirs(save_dir_model_params)

                if (step + 1) % save_params_interval == 0 or step == 0:
                    state = {'Encoder': self.encoder.state_dict(),
                             'Decoder': self.decoder.state_dict(),
                             'NoiseGenerator': self.noise_generator.state_dict(),
                             'Discriminator': self.discriminator.state_dict()
                             }

                    torch.save(state, save_dir_model_params + params_name + f'_{self.training_steps}.pth')
        ...
        for step in range(self.training_steps[1]):
            ...
            loss_list.append([loss_g.item(), loss_d.item(), loss_adv_g.item(), loss_adv_d.item(),
                              loss_la.item(), loss_rec.item(), loss_cls_real.item(), loss_cls_fake.item()])

            loss_la_list.append(loss_la.item())
            if len(loss_la_list) >= 50:
                loss_la_average = np.mean(loss_la_list)
                loss_la_list.pop(0)

            if report_loss:
                if (step + 1) % report_interval == 0 or step == 0 or \
                        (self.early_stop and loss_la_average <= self.alpha / 2 and step + 1 >= self.min_steps):
                    print("Step: %s" % step)
                    print("loss_G: %.4f, loss_D: %.4f" % (loss_g.item(), loss_d.item()))
            ...
            if save_loss:
                ...
                if (step + 1) % save_loss_interval == 0 or step == 0 or \
                        (self.early_stop and loss_la_average <= self.alpha / 2 and step + 1 >= self.min_steps):
                    with open(save_dir_loss + fname2 + f'_{self.training_steps}.csv', 'w', newline='') as csvfile:
                        csvwriter = csv.writer(csvfile)

                        csvwriter.writerow(['Loss_G', 'Loss_D', 'Loss_Adv_G', 'Loss_Adv_D',
                                            'Loss_La', 'Loss_Rec', 'Loss_Cls_Real', 'Loss_Cls_Fake'])
            ...
            if save_model_params:
                ...
                if (step + 1) % save_params_interval == 0 or step == 0 or \
                        (self.early_stop and loss_la_average <= self.alpha / 2 and step + 1 >= self.min_steps):
                    state = {'Encoder': self.encoder.state_dict(),
                             'Decoder': self.decoder.state_dict(),
                             'NoiseGenerator': self.noise_generator.state_dict(),
            ...
            if self.early_stop and loss_la_average <= self.alpha / 2 and step + 1 >= self.min_steps:
                print(f'Finished training at training steps {self.training_steps[0]}+{step + 1} !\n')
                break
    ...
    def eval(self, adata_list, slice_names=None, save_dir_data='../../results/', load_params=False,
             save_dir_model_params='../../results/', params_name='INSTINCT_params', save=False):
        self.encoder.eval()
        self.decoder.eval()
        ...
        if load_params:
            self.encoder.load_state_dict(torch.load(
                save_dir_model_params + params_name + f'_{self.training_steps}.pth')['Encoder'])
            self.decoder.load_state_dict(torch.load(
                save_dir_model_params + params_name + f'_{self.training_steps}.pth')['Decoder'])
            self.noise_generator.load_state_dict(torch.load(
                save_dir_model_params + params_name + f'_{self.training_steps}.pth')['NoiseGenerator'])
            self.discriminator.load_state_dict(torch.load(
                save_dir_model_params + params_name + f'_{self.training_steps}.pth')['Discriminator'])
