...
input_types = ['frag', 'bina']
labels2 = ['Fragment Count', 'Binary']
color_list_2 = ['lawngreen', 'yellow']
...
b_asws, b_pcrs, kbets, g_conns = [], [], [], []

for model in models:

    with open(save_dir + f'{mode}/comparison/{model}/{model}_results_dict.pkl', 'rb') as file:
...
if save:
    save_path = save_dir + f"{mode}/comparison/methods_clustering_performance_separate_scores_box_plot.pdf"
...
plt.gcf().subplots_adjust(left=0.15, top=0.9, bottom=0.15, right=None)

if save:
    save_path = save_dir + f"{mode}/comparison/methods_representation_quality_separate_scores_box_plot.pdf"
    plt.savefig(save_path)
plt.show()
...
if save:
    save_path = save_dir + f"{mode}/comparison/methods_batch_correction_separate_scores_box_plot.pdf"
    plt.savefig(save_path)
plt.show()
...
for k in range(len(metric_groups)):
    ...
    if save:
        save_path = save_dir + f"{mode}/comparison/methods_{metric_groups[k]}_score_box_plot.pdf"
        plt.savefig(save_path)
plt.show()
...
if save:
    save_path = save_dir + f"{mode}/comparison/methods_overall_score_box_plot.pdf"
    plt.savefig(save_path)
plt.show()
...
for k in range(len(mtx_list)):
    ...
    if save:
        ...
        if type_list[k] == 'mean':
            ...
            overview_matrix[:, 15] = np.median((np.mean(cluster_stacked, axis=-1) +
                                                np.mean(bio_conserve_stacked, axis=-1) +
                                                np.mean(batch_corr_stacked, axis=-1)) / 3, axis=-1)
        np.savetxt(save_dir + f'{mode}/comparison/methods_{type_list[k]}_separate_scores.txt', overview_matrix)
    ...
    if save:
        save_path = save_dir + f"{mode}/comparison/methods_{type_list[k]}_separate_scores_rank_plot.pdf"
        plt.savefig(save_path)
    plt.show()
...
for type in input_types:
    ...
    with open(save_dir + f'{mode}/comparison/INSTINCT_{type}/INSTINCT_{type}_results_dict.pkl', 'rb') as file:
        results_dict = pickle.load(file)
...
if save:
    save_path = save_dir + f"{mode}/comparison/input_types_clustering_performance_separate_scores_box_plot.pdf"
    plt.savefig(save_path)
plt.show()
...
for i, input_type in enumerate(input_types):
    ...
    positions = np.arange(2) * (len(input_types) + 1) + i
    boxplot = axs.boxplot(bio_conserve_stacked[i], positions=positions, patch_artist=True, widths=0.75,
                          labels=metric_list[6:8], showfliers=False, zorder=1)
...
axs.set_xticks([0.5 + 3 * i for i in range(2)])
axs.set_xticklabels(metric_list[6:8], fontsize=12)
axs.tick_params(axis='y', labelsize=12)
...
if save:
    save_path = save_dir + f"{mode}/comparison/input_types_representation_quality_separate_scores_box_plot.pdf"
    plt.savefig(save_path)
plt.show()
...
if save:
    save_path = save_dir + f"{mode}/comparison/input_types_batch_correction_separate_scores_box_plot.pdf"
    plt.savefig(save_path)
plt.show()
...
for k in range(len(metric_groups)):
    ...
    if save:
        save_path = save_dir + f"{mode}/comparison/input_types_{metric_groups[k]}_score_box_plot.pdf"
        plt.savefig(save_path)
plt.show()
...
if save:
    save_path = save_dir + f"{mode}/comparison/input_types_overall_score_box_plot.pdf"
    plt.savefig(save_path)
plt.show()
