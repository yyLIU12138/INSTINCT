...
# plot raw umap
raw_pca = np.load(save_dir + f'input_matrix_{mode}.npy')
...
if mode == 'S1':
    ...
    colors_for_slices = ['hotpink', 'goldenrod', 'cyan']
...
plt.legend(handles=legend_handles, fontsize=8, title='Slices', title_fontsize=10,
           loc='upper left')
plt.title(f'{mode} Slices (Raw)', fontsize=16)
if save:
    save_path = save_dir + f"{mode}/raw_slices_umap.pdf"
...
plt.tick_params(axis='both', bottom=False, top=False, left=False, right=False,
                labelleft=False, labelbottom=False, grid_alpha=0)
plt.title(f'{mode} Annotated Spot-types (Raw)', fontsize=16)
if save:
    save_path = save_dir + f"{mode}/raw_annotated_clusters_umap.pdf"
    plt.savefig(save_path)
# plt.show()
...
for j, model in enumerate(models):
    ...
    embed = pd.read_csv(save_dir + f'{mode}/comparison/{model}/{model}_embed_2.csv', header=None).values
    ...
    plot_mouseembryo_3(cas_list, adata_concat, 'clusters', 'matched_clusters', model, cluster_to_color_map,
                       matched_to_color_map, my_clusters, slice_name_list, sp_embedding,
                       save_root=save_dir+f'{mode}/comparison/', frame_color=colors_for_labels[j], save=save, plot=True)
