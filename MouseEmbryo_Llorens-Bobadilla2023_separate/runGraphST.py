...
if not os.path.exists(save_dir + f'{mode}/comparison/GraphST/'):
    os.makedirs(save_dir + f'{mode}/comparison/GraphST/')
...
for j in range(num_iters):
    ...
    if j == 0:
        ...
        for i in range(len(cas_list)):
            cas_list[i].write_h5ad(save_dir + f'{mode}/comparison/GraphST/filtered_{slice_name_list[i]}_aligned.h5ad')

    cas_list = [ad.read_h5ad(save_dir + f'{mode}/comparison/GraphST/filtered_{sample}_aligned.h5ad')
    ...
    with open(save_dir + f'{mode}/comparison/GraphST/GraphST_embed_{j}.csv', 'w', newline='') as file:
