...
if not os.path.exists('../../results/model_validity/MouseBrain_Jiang2023/sensitivity/k_neighbors/'):
    os.makedirs('../../results/model_validity/MouseBrain_Jiang2023/sensitivity/k_neighbors/')
...
for i in range(len(k_list)):
    ...
    print(f'The number of neighbors is {k_list[i]}\n')

    for k in range(num_iters):

        print(f'Iteration {k}')

        INSTINCT_model = INSTINCT_Model(cas_list, adata_concat, k=k_list[i], seed=1236+k, device=device)
        ...
        with open(f'../../results/model_validity/MouseBrain_Jiang2023/sensitivity/k_neighbors/k={k_list[i]}_embed_{k}.csv',
                  'w', newline='') as file:
